...
gpu_index=${1:?usage: launch_midlevel_v11_residual_variant_node31.sh GPU_INDEX LABEL INITIALIZER UPDATES LEARNING_RATE MARGIN_WEIGHT MIDDLE_WEIGHT HARD_WEIGHT HARD_QUANTILE SEED}
shift
...
exec env CUDA_VISIBLE_DEVICES="$gpu_index" bwrap \
  ...
  'set -euo pipefail
   ...
   exec bash scripts/train/run_midlevel_v11_residual_variant.sh "$@"' \
  bash "$@"
