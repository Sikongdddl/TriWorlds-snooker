...
gpu_index=${1:?usage: launch_midlevel_td3_evaluation_node31.sh GPU_INDEX CHECKPOINT [evaluation arguments...]}
shift
case "$gpu_index" in
  0|1|2|3|4|5|6|7) ;;
  *)
...
exec env CUDA_VISIBLE_DEVICES="$gpu_index" bwrap \
  ...
  'set -euo pipefail
   ...
   exec python scripts/tools/evaluate_midlevel_two_ball_td3_her.py "$@"' \
  bash "$@"
