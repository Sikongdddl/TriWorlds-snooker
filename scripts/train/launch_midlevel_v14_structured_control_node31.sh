...
gpu_index=${1:?usage: launch_midlevel_v14_structured_control_node31.sh GPU_INDEX MODE SPEED_REFERENCE}
mode=${2:?missing mode}
speed_reference=${3:?missing speed reference}
case "$gpu_index" in
  0|1|2|3|4|5|6|7) ;;
  *)
...
exec env CUDA_VISIBLE_DEVICES="$gpu_index" bwrap \
  ...
  'set -euo pipefail
   ...
   exec bash scripts/train/run_midlevel_v14_structured_control.sh "$@"' \
  bash "$mode" "$speed_reference"
