...
gpu_index=${1:?usage: launch_midlevel_v10_canonical_bc_variant_node31.sh GPU_INDEX LABEL EPOCHS BATCH_SIZE LEARNING_RATE FINAL_LEARNING_RATE SEED}
shift
...
exec env CUDA_VISIBLE_DEVICES="$gpu_index" bwrap \
  ...
  'set -euo pipefail
   ...
   exec bash scripts/train/run_midlevel_v10_canonical_bc_variant.sh "$@"' \
  bash "$@"
