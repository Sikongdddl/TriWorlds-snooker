...
gpu_index=${1:?usage: launch_midlevel_structured_speed_bc_node31.sh GPU_INDEX [training arguments...]}
shift
case "$gpu_index" in
  0|1|2|3|4|5|6|7) ;;
  *)
...
exec env CUDA_VISIBLE_DEVICES="$gpu_index" bwrap \
  ...
  'set -euo pipefail
   ...
   exec python scripts/train/train_midlevel_structured_speed_bc.py "$@"' \
  bash "$@"
