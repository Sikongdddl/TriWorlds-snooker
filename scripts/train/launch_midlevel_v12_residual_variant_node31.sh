...
gpu_index=${1:?usage: launch_midlevel_v12_residual_variant_node31.sh GPU_INDEX LABEL INITIALIZER UPDATES LEARNING_RATE PHYSICAL_WEIGHT INTERVAL_WEIGHT HARD_METRIC HARD_WEIGHT SEED [BASELINE_DETAILS_LABEL]}
shift
...
exec env CUDA_VISIBLE_DEVICES="$gpu_index" bwrap \
  ...
  'set -euo pipefail
   ...
   exec bash scripts/train/run_midlevel_v12_residual_variant.sh "$@"' \
  bash "$@"
