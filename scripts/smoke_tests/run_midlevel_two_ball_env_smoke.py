...
def _fixture_task(simulator: TwoBallShotSimulator) -> TwoBallTask:
    ...
    return TwoBallTask(
        ...
        target_stop_position=np.array(
            [0.63129483, 0.00240096], dtype=np.float64
        ),
        ...
        candidate_seed=1,
        elapsed_time=4.42440000001205,
        min_object_pocket_distance=0.015478149617568205,
        event_metrics={
            "correct_pot": True,
...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="midlevel-two-ball-smoke-") as directory:
        ...
        try:
            MidLevelTwoBallEnv(task_path, max_time=7.0)
        except ValueError as error:
        ...
        env = MidLevelTwoBallEnv(task_path)
