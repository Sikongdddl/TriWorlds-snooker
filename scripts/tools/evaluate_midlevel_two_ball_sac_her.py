...
def main() -> None:
    ...
    policy = SingleStepTD3BC.load(str(args.checkpoint), device=args.device)
    checkpoint_manifest = getattr(policy, "midlevel_training_manifest", None)
    if not isinstance(checkpoint_manifest, dict):
        raise ValueError("Evaluation checkpoint has no training manifest.")
    checkpoint_algorithm = checkpoint_manifest.get("algorithm")
    if not isinstance(checkpoint_algorithm, dict):
        raise ValueError("Evaluation checkpoint has no algorithm manifest.")
    checkpoint_algorithm_version = checkpoint_algorithm.get("version")
    if checkpoint_algorithm_version != SINGLE_STEP_TD3_VERSION:
        raise ValueError(
    ...
    stopped_correct_mask = np.asarray(
        [
    ...
    stopped_correct_errors = stop_errors[stopped_correct_mask]
    pocket_reports: dict[str, dict[str, float | int]] = {}
    pocket_names = sorted({str(info["pocket_name"]) for info in infos})
    for pocket_name in pocket_names:
        pocket_indices = [
            index
            for index, info in enumerate(infos)
            if str(info["pocket_name"]) == pocket_name
        ]
        pocket_reports[pocket_name] = {
            "task_count": len(pocket_indices),
            "correct_pot_rate": float(
                np.mean([bool(infos[index]["correct_pot"]) for index in pocket_indices])
            ),
            "joint_success_rate": float(
                np.mean([bool(infos[index]["joint_success"]) for index in pocket_indices])
            ),
            "scratch_rate": float(
                np.mean([bool(infos[index]["cue_scratch"]) for index in pocket_indices])
            ),
        }
    report = {
        ...
        "stopped_correct_pot_rate": float(np.mean(stopped_correct_mask)),
        "mean_stop_error_given_stopped_correct_pot_m": (
            float(np.mean(stopped_correct_errors))
            if stopped_correct_errors.size > 0
            else None
        ),
